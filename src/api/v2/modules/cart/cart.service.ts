...
import {
  ...
  findCartItem,
  increaseCartItemQty,
} from "./cartItem/cartItem.service";
...
async function addToCartOne() {
  const userId = "";
  const productItem = await prisma.product_item.findUniqueOrThrow({
    where: { id: "" },
  });

  const userCart = await prisma.shopping_cart.findFirstOrThrow({
    where:{userId}
  })

  if (!productItem) {
    throw Error("Sorry,product item does not exist");
  }
  if (productItem.qty_in_stock === 0) {
  ...
  try {
    ...
    const cart = await prisma.shopping_cart.create({
      data: {
        userId,
        shopping_cart_item: {
          create: [{ product_item_id: itemId, qty: 1 }],
        },
      },
      include: {
        shopping_cart_item: true,
